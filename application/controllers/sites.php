...
class sites extends CI_Controller {
	...
	public function edit($id){
		...
		$cylinder = $this->site_data->fetch_coa();
		$lcs = $this->site_data->fetch_site_standards($id,'LCS');
		$cvs = $this->site_data->fetch_site_standards($id,'CVS');
		$data['lcs_page'] = $this->load->view($controller.'/lcs', array('lcs' => $lcs, 'controller' => $controller, 'site_id'=> $id, 'cylinder' => $cylinder), true);
		$data['cvs_page'] = $this->load->view($controller.'/cvs', array('cvs' => $cvs, 'controller' => $controller, 'site_id'=> $id, 'cylinder' => $cylinder), true);
	...
	public function ajax_fetch_lcs_standard($id,$type){
		...
		$sql = "SELECT s.*, c.cylinder FROM `standards` s LEFT JOIN `coa` c ON s.coa_id=c.id WHERE s.id=$id AND type='$type'";
		$q = $this->db->query($sql);
		$data = $q->result_array();
