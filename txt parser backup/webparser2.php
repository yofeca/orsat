...
function fetch_txo($filepath){
	...
	$sql = "SELECT id FROM `txo_dumps` WHERE `filename`='" . mysql_real_escape_string($filename) . "' LIMIT 1";
	$r = dbQuery($sql);
	return $r[0]['id'];
}
...
function parseTXO($filepath){ //filepath should be absolute path of the file
	...
	$txo_id = fetch_txo($filepath); //fetch txo
	...
	for($i=1; $i<$totalcsv; $i++){
		...
		$sql .="`filename_id` = '" .mysql_real_escape_string($file_year.trim($filename)). "',
		...
		for($i=$header_counter+6; $i<$totalcsv; $i++){
			...
			if($subtotalcsv_count > 4 && (trim($csv[$i][0]) || trim($csv[$i][1]))){
				...
				$sql .="`peak` = '".mysql_real_escape_string(trim($csv[$i][0]))."',
				...
				if( $subtotalcsv_count > 4 ){
					if($csv[$i][2] == "------" && $csv[$i][4] == "------"){
						...
						$sql .="`filename` = '".mysql_real_escape_string(trim($filename))."',
						...
						$sql .=", `ascii_file` = '" . mysql_real_escape_string(trim($csv[$totalcsv-1][1])). "'";
						if($txo_id){
							$sql .= " WHERE `txo_dump_id`='".$txo_id."'";
						}else{
							$sql .= ",`txo_dump_id` = '" . $txo_insert_id . "'";
