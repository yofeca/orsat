...
function fetch_site($instrument_name){	
	...
	$sql = "SELECT id FROM `network` WHERE `name`='" . strtoupper($instrument_name) . "' LIMIT 1";
	$r = dbQuery($sql);
	
	if($r){
		return $r[0]['id'];
	}else{
		$sql = "INSERT INTO `network` SET `name`= '".mysql_real_escape_string(strtoupper($instrument_name))."'";
		$r = dbQuery($sql);
...
function fetch_sample_type($filename,$sample_name){
	...
	$sql = "SELECT id FROM `sample_types` WHERE `name`='" . strtoupper($sample_name) . " OR file_designator=" . $file_designator . "' LIMIT 1";
	$r = dbQuery($sql);
			...
			for($j=0; $j<$t; $j++){
				...
				if(substr($index, -5, 5) != "====="){
					...
					if(substr($index, -1, 1)==":"){
						...
						if(strtolower($key)=='data_acquisition_time') $value = convert_to_date($value);
						...
						if(strtolower($key)=='data_acquisition_time') $data_acquisition_time = mysql_real_escape_string(trim($value));
